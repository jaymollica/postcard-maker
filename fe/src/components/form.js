			...
			verify(lob_publishable_api_key_live, address).then((verificationResult) => {
				...
				if( verificationResult.valid_address && verificationResult.deliverability !== 'undeliverable' ){
					...
					setBillingDetails({
						...
						name: recipientName  // This is the recipient's name for Lob
					});
				}
				else{
	...
	if( postcardGenerated ){
		return (
			<div className={`addressform ${addressVerified ? 'verified' : (verifying ? 'verifying' : '')}` }>
				...
				<div className="description">Enter the recipient's name and address where you'd like your postcard delivered. We'll verify it to ensure successful delivery.</div>
				...
				>
					<div style={{display: 'flex', flexDirection: 'row', marginBottom: '1em'}}>
						<label style={{alignSelf: 'center', minWidth: '5em', marginRight: '1em', textAlign: 'end'}} htmlFor='recipient_name'>
							Recipient name
						</label>
						<input
							style={{ 
